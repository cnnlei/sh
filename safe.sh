...
# ======================================================================
#  专业多链交互式 IPTables 管理器 (v36.3 - 智能守护版)
# ======================================================================
#  - 版本说明 (v36.3):
#    - [语言] 所有用户界面已完全转换为简体中文。
#    - [智能守护] 新增核心出站规则守护功能，防止误删导致网络中断。
#    - [规则即时生效] 在任何增删规则后，自动清除已建立的连接（并保留当前SSH会话），使规则立即对所有流量生效。
#    - [功能增强] IP黑白名单、GeoIP封锁/许可功能，均已支持按端口（单个/多个/范围）进行设置，实现更精细的访问控制。
#    - [智慧修复] Docker模块的健康检查功能升级，在检测到规则丢失时，会尝试自动重启Docker服务进行修复。
#    - [兼容性] “重置防火墙”功能现在会自动重启Docker服务，以确保网络规则的兼容性。
# ======================================================================
...
function select_protocol() {
    clear
    echo "======================================================"
    echo "        IPTables 防火墙管理器 V36.3 (智能守护版)"
    echo "======================================================"
    echo "请选择您要管理的防火墙协议："
    echo " 1. IPv4 (全功能)"
...
function ensure_output_rule_exists() {
    ...
    if ! $IPTABLES_CMD -C ${core_rule} > /dev/null 2>&1; then
        ...
        # 在 OUTPUT 链的第 1 条位置 (-I OUTPUT 1) 插入规则
        $IPTABLES_CMD -I ${core_rule}

        # 仅在恢复时给出提示并自动保存，确保持久化
        echo -e "\n${YELLOW}警告：检测到核心出站规则丢失，已自动恢复至第一条并保存。${NC}"
        save_all_rules
        sleep 2 # 暂停2秒，确保用户能看到提示
    fi
}
...
function flush_connections_safely() { if ! command -v conntrack &>/dev/null; then return; fi; echo -e "\n${YELLOW}正在清除已建立的连接 (保留当前SSH会话)...${NC}"; local ssh_port; ssh_port=$(ss -tn | grep -F "ESTAB" | grep -F "$(echo "$SSH_CONNECTION" | awk '{print $1}')" | awk '{print $4}' | awk -F: '{print $NF}' | head -n 1); if [[ -z "$ssh_port" ]]; then echo -e "${RED}无法确定当前 SSH 会话端口，为安全起见，跳过连接清除。${NC}"; return; fi; conntrack -D -p tcp --dport-neq "$ssh_port" &>/dev/null; conntrack -D -p udp &>/dev/null; echo -e "${GREEN}✓ 非 SSH 连接已清除。${NC}"; }
...
function add_to_list() { local list_type=$1; local chain_name=$2; local jump_action=$3; read -p "请输入要添加到${list_type}的 ${IP_VERSION} IP (单个或CIDR): " ip; if [[ -z "$ip" ]]; then echo -e "\n${RED}错误：IP 不能为空。${NC}"; return 1; fi; local valid_format=false; if [[ "$IP_VERSION" == "IPv4" ]]; then local ipv4_regex='^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(\/(3[0-2]|[12]?[0-9]))?$' ; if echo "$ip" | grep -qE "$ipv4_regex"; then valid_format=true; fi; else if [[ "$ip" == *":"* && "$ip" != *" "* ]]; then valid_format=true; fi; fi; if ! $valid_format; then echo -e "\n${RED}错误：'${ip}' 格式无效。${NC}"; return 1; fi; read -p "端口 (单个, 多个80,443, 范围3000:4000) [回车默认所有端口]: " ports; local rule_added=0; if [[ -n "$ports" ]]; then for proto in tcp udp; do local port_param; if [[ "$ports" == *","* || "$ports" == *":"* ]]; then port_param="-m multiport --dports $ports"; else port_param="--dport $ports"; fi; if $IPTABLES_CMD -A "$chain_name" -s "$ip" -p "$proto" $port_param -j "$jump_action"; then rule_added=$((rule_added+1)); fi; done; else if $IPTABLES_CMD -A "$chain_name" -s "$ip" -j "$jump_action"; then rule_added=$((rule_added+1)); fi; fi; if [ $rule_added -gt 0 ]; then echo -e "${GREEN}✓ 规则添加成功。${NC}"; save_all_rules; flush_connections_safely; else echo -e "\n${RED}✗ 添加规则失败！${NC}"; return 1; fi; }
function manage_port_rule() { local chain=$1; local action=$2; local action_text=$3; read -p "源 IP (留空则所有): " ip; read -p "协议: 1.TCP 2.UDP 3.两者 [3]: " proto_choice; local protos=(); case $proto_choice in 1) protos=("tcp");; 2) protos=("udp");; ""|3) protos=("tcp" "udp");; *) echo -e "${RED}无效。${NC}"; return;; esac; read -p "端口(可输入单个80, 多个80,443, 或范围3000:4000): " port; if [[ -z "$port" ]]; then echo -e "${RED}端口不能为空。${NC}"; return; fi; for proto in "${protos[@]}"; do local rule_added=false; if [[ "$port" == *":"* ]]; then if $IPTABLES_CMD -A "$chain" ${ip:+-s "$ip"} -p "$proto" -m multiport --dports "$port" -j "$action"; then rule_added=true; fi; elif [[ "$port" == *","* ]]; then if $IPTABLES_CMD -A "$chain" ${ip:+-s "$ip"} -p "$proto" -m multiport --dports "$port" -j "$action"; then rule_added=true; fi; else if $IPTABLES_CMD -A "$chain" ${ip:+-s "$ip"} -p "$proto" --dport "$port" -j "$action"; then rule_added=true; fi; fi; if $rule_added; then echo -e "${GREEN}✓ 已添加规则...${NC}"; fi; done; save_all_rules; flush_connections_safely; }
...
# --- [新模块] Docker 网络策略管理器 的辅助函数 ---

# 初始化策略链 (优化版：挂载到 DOCKER-USER)
function docker_init_chain() {
    local IPTABLES="iptables-legacy"
    local CHAIN_NAME="DOCKER_NET_POLICY"
    # 确保我们的自定义链存在
    $IPTABLES -N $CHAIN_NAME 2>/dev/null
    
    # 检查 Docker 的 DOCKER-USER 链是否存在，这是 Docker 正常工作的前提
    if ! $IPTABLES -L DOCKER-USER >/dev/null 2>&1; then
        echo -e "${RED}错误：未找到 Docker 核心链 DOCKER-USER，请确保 Docker 服务正常运行！${NC}"
        return 1
    fi

    # 将我们的自定义链挂载到 DOCKER-USER 的最顶端，这是 Docker 官方推荐的用户自定义规则入口
    if ! $IPTABLES -C DOCKER-USER -j $CHAIN_NAME >/dev/null 2>&1; then
        echo -e "${BLUE}[+] 正在将 ${CHAIN_NAME} 挂载到 Docker 官方链 (DOCKER-USER)...${NC}"
...
function docker_select_container() {
    ...
    local -a container_ids container_names
    mapfile -t container_ids < <($DOCKER_CMD ps --format "{{.ID}}")
    mapfile -t container_names < <($DOCKER_CMD ps --format "{{.Names}}")

    if [ ${#container_ids[@]} -eq 0 ]; then
        echo -e "${YELLOW}⚠ 没有侦测到正在运行的容器，请先启动容器再试。${NC}" >&2
        return 1
    fi
    ...
    for ((i=0; i<${#container_ids[@]}; i++)); do
        printf " %2d) %-20s ${CYAN}[%s]${NC}\n" "$((i+1))" "${container_names[$i]}" "${container_ids[$i]:0:12}" >&2
    done
    echo "--------------------------------------" >&2
    ...
    echo "${container_ids[$((choice-1))]}""_""${container_names[$((choice-1))]}"
    return 0
}

# 添加规则
function docker_add_rule() {
    local IPTABLES="iptables-legacy"
    local CHAIN_NAME="DOCKER_NET_POLICY"
    local DOCKER_CMD
    DOCKER_CMD=$(command -v docker)
    ...
    container_info=$(docker_select_container) || return
    
    local cid="${container_info%_*}"
    local cname="${container_info#*_}"

    local ip
    ip=$($DOCKER_CMD inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' "$cid")
    if [ -z "$ip" ]; then
        echo -e "${RED}⚠ 无法获取容器 ${cname} 的 IP 地址。${NC}"
        return 1
    fi
    echo -e "${BLUE}已选择容器: ${YELLOW}${cname}${NC} (内部 IP: ${CYAN}${ip}${NC})"

    # --- 新增：选择流量方向 ---
    local direction dir_tag
    echo "请选择策略的流量方向:"
    echo " 1. 入站 (外部网络访问此容器)"
    echo " 2. 出站 (此容器访问外部网络)"
    read -p "请输入选择 [回车默认为 所有方向]: " direction
    
    local rule_part_s rule_part_d
    case "$direction" in
        1) dir_tag="IN" ;;
        2) dir_tag="OUT" ;;
        "") dir_tag="ALL" ;;
        *) echo -e "${RED}无效的方向选择。${NC}"; return 1 ;;
    esac

    read -p "请输入远程IP (单个或CIDR, 留空则为 'any'): " remote_ip
    remote_ip=${remote_ip:-"0.0.0.0/0"}

    read -p "目标端口 (单个, 逗号分隔或范围, 留空则为 'all'): " ports
    ports=${ports:-"all"}

    local proto_choice proto
    echo "协议选择: 1=TCP, 2=UDP, [回车默认为 ALL]"
    read -p "请选择协议: " proto_choice
    case "$proto_choice" in
        1) proto="tcp" ;;
        2) proto="udp" ;;
        *) proto="all" ;;
    ...
    local rules_to_add=()
    if [[ "$dir_tag" == "IN" || "$dir_tag" == "ALL" ]]; then
        rules_to_add+=("-s ${remote_ip} -d ${ip}")
    fi
    if [[ "$dir_tag" == "OUT" || "$dir_tag" == "ALL" ]]; then
        rules_to_add+=("-s ${ip} -d ${remote_ip}")
    fi

    local rule_added_count=0
    for base_rule in "${rules_to_add[@]}"; do
        local rule="$base_rule"
        [ "$proto" != "all" ] && rule="$rule -p $proto"
        
        if [ "$ports" != "all" ]; then
            if [ "$proto" = "all" ]; then
                echo -e "${YELLOW}⚠ 必须选择特定协议 (TCP/UDP) 才能限制端口，已跳过此条规则。${NC}" >&2
                continue
            fi
            if [[ "$ports" == *","* ]]; then
                rule="$rule -m multiport --dports $ports"
            elif [[ "$ports" == *":"* ]]; then
                 rule="$rule -m multiport --dports $ports"
            else
                rule="$rule --dport $ports"
            fi
        fi

        # 根据方向确定comment中的标签
        local effective_dir_tag=""
        if [[ "$base_rule" == *"-d ${ip}"* ]]; then
            effective_dir_tag="IN"
        else
            effective_dir_tag="OUT"
        fi

        local comment="docker:${effective_dir_tag}:${cname}(${cid:0:12})"
        if $IPTABLES -A $CHAIN_NAME $rule -j $action -m comment --comment "$comment"; then
            echo -e "${GREEN}✓ 已添加 ${action_color}${action}${NC} 策略: ${rule} --comment ${comment}${NC}"
            rule_added_count=$((rule_added_count + 1))
        else
            echo -e "${RED}[!] 添加策略失败: ${rule}${NC}"
        fi
    done
    
    if [ $rule_added_count -gt 0 ]; then
        save_all_rules # 集成到主脚本的保存功能
...
# 查看规则
function docker_show_rules() {
    local IPTABLES="iptables-legacy"
    local CHAIN_NAME="DOCKER_NET_POLICY"
    clear
    echo "======================================================"
    echo -e "          ${CYAN}当前 Docker 防火墙策略 (${CHAIN_NAME})${NC}"
    echo "======================================================"
    $IPTABLES -L $CHAIN_NAME -n --line-numbers -v
    echo "======================================================"
}

# 删除规则
function docker_delete_rule() {
    local IPTABLES="iptables-legacy"
    local CHAIN_NAME="DOCKER_NET_POLICY"
    docker_show_rules
    local rule_count
    rule_count=$($IPTABLES -L $CHAIN_NAME -n --line-numbers 2>/dev/null | sed '1,2d' | wc -l)
    if [ "$rule_count" -eq 0 ]; then
        echo -e "\n${YELLOW}当前没有任何规则可删除。${NC}"
        return
    fi

    echo " a) 删除此链中的全部规则"
    read -p "请输入要删除的规则编号 (或 a): " num
    
    if [[ "$num" == "a" || "$num" == "A" ]]; then
        $IPTABLES -F $CHAIN_NAME
        echo -e "${GREEN}[✓] 已清空 ${CHAIN_NAME} 链的所有规则。${NC}"
        save_all_rules # 集成到主脚本的保存功能
        flush_connections_safely
    else
        # 获取总行数，以防止误删
        local total_lines_for_check
        total_lines_for_check=$($IPTABLES -S $CHAIN_NAME 2>/dev/null | wc -l)
        if ! [[ "$num" =~ ^[0-9]+$ ]] || [ "$num" -lt 1 ] || [ "$num" -gt "$total_lines_for_check" ]; then
            echo -e "${RED}⚠ 无效的规则编号！${NC}"
            return 1
        fi
        if $IPTABLES -D $CHAIN_NAME "$num"; then
...
# --- [主集成函数] Docker 网络管理主菜单 (优化版) ---
function manage_docker_menu() {
    # 仅在 IPv4 模式下工作
    if [[ "$IP_VERSION" != "IPv4" ]]; then
        echo -e "${RED}错误: Docker 网络管理目前仅在 IPv4 模式下受支持。${NC}"; sleep 2
        return
    fi
    # --- 环境检查 ---
    if ! command -v iptables-legacy >/dev/null 2>&1; then
        echo -e "${RED}错误: 此模块需要 'iptables-legacy'，但命令未找到。${NC}"; sleep 3
        return
    fi
    if ! command -v docker >/dev/null 2>&1; then
        echo -e "${RED}错误：无法在您的 PATH 中找到 'docker' 命令。${NC}"; sleep 3
        return 1
    fi
    # --- [智慧诊断与自动修复] Docker 核心规则健康检查 ---
    if ! iptables-legacy-save | grep -q -- "-A FORWARD -j DOCKER-USER"; then
        clear
        echo -e "${RED}======================================================${NC}"
        echo -e "${YELLOW}      警告：侦测到 Docker 的核心防火墙规则丢失！${NC}"
        echo -e "${RED}======================================================${NC}"
        echo
        echo -e "${BLUE}正在尝试自动修复 (重启 Docker 服务)...${NC}"
        systemctl restart docker
        sleep 3 # 等待 Docker 服务和网络规则初始化
        if iptables-legacy-save | grep -q -- "-A FORWARD -j DOCKER-USER"; then
            echo -e "${GREEN}✓ 自动修复成功！Docker 网络规则已恢复。${NC}"
            sleep 2
        else
            echo -e "${RED}✗ 自动修复失败！Docker 服务重启后规则依然缺失。${NC}"
            echo -e "${YELLOW}请退出本脚本，手动检查 Docker 服务状态及系统 iptables 版本设置。${NC}"
    ...
    while true; do
        clear
        echo "======================================"
        echo -e "      ${CYAN}Docker 网络防火墙管理${NC}"
        echo "======================================"
        echo -e " 1. 添加 ${GREEN}允许 (ACCEPT)${NC} 策略"
        echo -e " 2. 添加 ${RED}拒绝 (DROP)${NC} 策略"
        echo " 3. 查看当前所有策略"
        echo " 4. 删除指定策略"
        echo "--------------------------------------"
        echo " q. 返回主菜单"
        ...
        case "$choice" in
            1) docker_add_rule ACCEPT ;;
            2) docker_add_rule DROP ;;
            3) docker_show_rules; read -n 1 -s -r -p $'\n按任意键返回...';;
            4) docker_delete_rule ;;
            q|Q) break ;;
...
function restore_rules() {
    ...
    if ! tar -xzf "$selected_backup_file" -C "$tmp_dir"; then
        echo -e "${RED}✗ 解压缩备份文件失败！文件可能已损坏。${NC}"
        rm -rf "$tmp_dir"
        return 1
    fi
    ...
    if $restore_ok; then
        echo -e "\n${GREEN}✓ 规则恢复成功！正在储存以确保持久化...${NC}"; save_all_rules
    else
...
function install_fail2ban_f2b() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ "$DISTRO" == "debian" ]; then
            ...
            if ! grep -q "^\s*backend\s*=" "$JAIL_LOCAL" 2>/dev/null; then sed -i "/\[DEFAULT\]/a backend = systemd" "$JAIL_LOCAL"; echo -e "${GREEN}✓ 已在 ${JAIL_LOCAL} 中设置 backend = systemd。${NC}"; else echo -e "${YELLOW}侦测到 backend 已在 ${JAIL_LOCAL} 中设置，跳过自动设置。${NC}"; fi
        ...
        echo -e "${BLUE}正在启动并设置开机自启 (使用 restart 确保新设置生效)...${NC}"; systemctl enable fail2ban; systemctl restart fail2ban; echo -e "${GREEN}✓ Fail2ban 已启动。${NC}"
...
function add_chain_rule() { local chain=$1; if [[ "$chain" == "FORWARD" ]]; then if ! check_and_enable_forwarding; then return 1; fi; fi; echo "--- 在 $chain 链中新增新规则 ($IP_VERSION) ---"; echo "请选择规则的目标 (Target):"; echo " 1. ACCEPT (允许)"; echo " 2. DROP  (丢弃 - 静默)"; echo " 3. REJECT (拒绝 - 告知对方)"; read -p "请输入选择 [1-3]: " target_choice; local target; case $target_choice in 1) target="ACCEPT" ;; 2) target="DROP" ;; 3) target="REJECT" ;; *) echo -e "${RED}无效选择。${NC}"; return 1 ;; esac; echo "请选择协议 (Protocol):"; echo " 1. all (全部协议)"; echo " 2. tcp"; echo " 3. udp"; echo " 4. ${ICMP_PROTO}"; read -p "请输入选择 [回车默认为 1 (all)]: " proto_choice; local proto; case $proto_choice in ""|1) proto="all" ;; 2) proto="tcp" ;; 3) proto="udp" ;; 4) proto="${ICMP_PROTO}" ;; *) echo -e "${RED}无效选择。${NC}"; return 1 ;; esac; read -p "源地址 (IP/CIDR, 留空为 any): " src; read -p "目的地址 (IP/CIDR, 留空为 any): " dst; local dport=""; if [[ "$proto" == "tcp" || "$proto" == "udp" ]]; then read -p "目的端口 (留空为 any): " dport; fi; local rule_cmd="$IPTABLES_CMD -A $chain"; if [[ "$proto" != "all" ]]; then rule_cmd+=" -p $proto"; fi; if [[ ! -z "$src" ]]; then rule_cmd+=" -s $src"; fi; if [[ ! -z "$dst" ]]; then rule_cmd+=" -d $dst"; fi; if [[ ! -z "$dport" ]]; then rule_cmd+=" --dport $dport"; fi; rule_cmd+=" -j $target"; echo -e "${BLUE}将要执行: $rule_cmd${NC}"; if eval "$rule_cmd"; then echo -e "${GREEN}✓ 规则新增成功。${NC}"; save_all_rules; else echo -e "${RED}✗ 规则新增失败。${NC}"; fi; }
...
function add_port_forward_rule() { if [[ "$IP_VERSION" == "IPv6" ]]; then echo -e "\n${RED}错误：此功能仅在 IPv4 模式下可用。${NC}"; sleep 2; return 1; fi; if ! check_and_enable_forwarding; then return 1; fi; clear; echo "--- 新增新的端口转发 (DNAT) 规则 (IPv4) ---"; local public_ip private_ip public_port private_port proto_choice; local public_ip_param=""; local ipv4_regex='^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$'; read -p "公网目标 IP (留空则代表所有IP): " public_ip; if [[ -n "$public_ip" ]]; then if ! [[ "$public_ip" =~ $ipv4_regex ]]; then echo -e "\n${RED}错误：公网IP地址格式无效。请输入单一IPv4地址。${NC}"; return 1; fi; public_ip_param="-d $public_ip"; fi; read -p "公网端口 (1-65535): " public_port; if ! [[ "$public_port" =~ ^[0-9]+$ && "$public_port" -ge 1 && "$public_port" -le 65535 ]]; then echo -e "\n${RED}错误：公网端口号无效。必须是 1-65535 之间的数字。${NC}"; return 1; fi; read -p "内网目标 IP: " private_ip; if ! [[ "$private_ip" =~ $ipv4_regex ]]; then echo -e "\n${RED}错误：内网目标IP地址格式无效。${NC}"; return 1; fi; read -p "内网目标端口 [回车同公网端口]: " private_port; [[ -z "$private_port" ]] && private_port=$public_port; if ! [[ "$private_port" =~ ^[0-9]+$ && "$private_port" -ge 1 && "$private_port" -le 65535 ]]; then echo -e "\n${RED}错误：内网目标端口号无效。必须是 1-65535 之间的数字。${NC}"; return 1; fi; echo "请选择协议:"; echo " 1. TCP"; echo " 2. UDP"; echo " 3. 两者 (TCP+UDP)"; read -p "请输入选择 [回车默认为 3]: " proto_choice; local protos=(); case $proto_choice in 1) protos=("tcp") ;; 2) protos=("udp") ;; ""|3) protos=("tcp" "udp") ;; *) echo -e "\n${RED}无效选择。${NC}"; return 1 ;; esac; local rules_added=0; for proto in "${protos[@]}"; do echo -e "\n${BLUE}--- 正在为协议 ${proto^^} 新增规则 ---${NC}"; local comment_tag="dnat_${proto}_${public_port}_to_${private_ip}_${private_port}"; local to_destination_format="${private_ip}:${private_port}"; echo "  -> 新增 DNAT 规则 (nat 表)..."; if $IPTABLES_CMD -t nat -A PREROUTING $public_ip_param -p "$proto" --dport "$public_port" -j DNAT --to-destination "$to_destination_format" -m comment --comment "$comment_tag"; then echo "  -> 新增 FORWARD 规则 (filter 表)..."; $IPTABLES_CMD -A FORWARD -d "$private_ip" -p "$proto" --dport "$private_port" -m state --state NEW,RELATED,ESTABLISHED -j ACCEPT -m comment --comment "$comment_tag"; rules_added=$((rules_added + 1)); else echo -e "${RED}       ✗ 新增 DNAT 规则失败！可能是规则已存在或参数冲突。${NC}"; fi; done; if [ $rules_added -gt 0 ]; then echo -e "\n${YELLOW}注意: 为使转发流量能正确返回，通常需要设置 SNAT 或 MASQUERADE。${NC}"; read -p "是否需要为此转发新增通用的 MASQUERADE 伪装规则? [y/n]: " add_masq; if [[ "$add_masq" == "y" || "$add_masq" == "Y" ]]; then local public_iface; public_iface=$(ip -o -${SS_FAMILY_FLAG:1} route get 8.8.8.8 2>/dev/null | grep -oP 'dev \K\S+'); if [[ -z "$public_iface" ]]; then echo -e "${RED}无法自动侦测到公网接口!${NC}"; else if ! $IPTABLES_CMD -t nat -C POSTROUTING -o "$public_iface" -j MASQUERADE 2>/dev/null; then echo -e "${BLUE}正在为出口 ${public_iface} 新增一条通用的 MASQUERADE 规则...${NC}"; $IPTABLES_CMD -t nat -A POSTROUTING -o "$public_iface" -j MASQUERADE -m comment --comment "generic-masquerade-for-forwarding"; else echo -e "${YELLOW}侦测到已存在一条通用的 MASQUERADE 规则，无需重复新增。${NC}"; fi; fi; fi; echo -e "\n${GREEN}✓ 端口转发规则集新增完成。${NC}"; save_all_rules; else echo -e "\n${YELLOW}未新增任何新规则。${NC}"; fi; }
...
function show_unified_menu() {
    ...
    echo "======================================================"; echo -e "        IPTables 防火墙管理器 (V36.3 - ${YELLOW}${IP_VERSION} 智能守护模式${NC})"; echo -e " (防火墙: ${fw_status} | SSH:${ssh_port} | 策略:${policy_color}${policy}${NC} | 转发:${fwd_color}${fwd_status}${NC})"; echo "======================================================"
...
# 主循环
while true; do
    ensure_output_rule_exists # <-- 激活专注版的 OUTPUT 规则守护功能
    show_unified_menu
    case $choice in
