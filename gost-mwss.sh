...
function add_new_service() {
    ...
    read -p ">> 请输入监听 IP 地址 [默认: :: (自动支持IPv4+IPv6)]: " GOST_LISTEN_IP < /dev/tty
    
    if [ -z "$GOST_LISTEN_IP" ]; then
        GOST_LISTEN_IP="::"
    fi

    read -p ">> 请输入 Gost-MWSS 服务的【用户名】: " GOST_USER < /dev/tty
    read -p ">> 请输入 Gost-MWSS 服务的【密码】: " GOST_PASSWORD < /dev/tty
    read -p ">> 请输入 Gost-MWSS 服务的【端口号】: " GOST_PORT < /dev/tty
    read -p ">> 请输入 Gost-MWSS 服务的【路径】 (path): " GOST_PATH < /dev/tty
    read -p ">> 请输入用于'knock'功能的域名 [默认: www.yaohuo.me]: " GOST_KNOCK_DOMAIN < /dev/tty
    GOST_KNOCK_DOMAIN=${GOST_KNOCK_DOMAIN:-www.yaohuo.me}

    local GOST_FORWARD_PARAM=""; local FORWARD_PROXY_ADDRESS=""
    echo -e "${YELLOW}--- 高级选项: 后置转发 (可选) ---${NC}"
    read -p ">> 是否要为此服务添加一个后置转发代理? [y/N]: " ADD_FORWARDER < /dev/tty
    if [[ "$ADD_FORWARDER" == "y" || "$ADD_FORWARDER" == "Y" ]]; then
        read -p ">> 请输入完整的后置转发代理地址 (例如: socks5://127.0.0.1:1080): " FORWARD_PROXY_ADDRESS < /dev/tty
    ...
    read -p ">> 请输入证书(crt)文件路径 [默认: /root/1.crt]: " GOST_CERT_FILE < /dev/tty
    GOST_CERT_FILE=${GOST_CERT_FILE:-/root/1.crt}

    read -p ">> 请输入密钥(key)文件路径 [默认: /root/1.key]: " GOST_KEY_FILE < /dev/tty
    ...
    if [ ! -f "$GOST_CERT_FILE" ] || [ ! -f "$GOST_KEY_FILE" ]; then
        ...
        read -p "是否继续? (y/n): " confirm_cert < /dev/tty
        if [[ "$confirm_cert" != "y" ]]; then return; fi
    fi
...
function modify_service() {
    ...
    read -p ">> 监听 IP 地址 [当前: ${CURRENT_IP}]: " NEW_LISTEN_IP < /dev/tty
    NEW_LISTEN_IP=${NEW_LISTEN_IP:-$CURRENT_IP}
    
    read -p ">> 用户名 [当前: ${CURRENT_USER}]: " NEW_USER < /dev/tty
    NEW_USER=${NEW_USER:-$CURRENT_USER}
    
    read -p ">> 密码 [当前: ${CURRENT_PASS}]: " NEW_PASSWORD < /dev/tty
    NEW_PASSWORD=${NEW_PASSWORD:-$CURRENT_PASS}
    
    read -p ">> 端口号 [当前: ${CURRENT_PORT}]: " NEW_PORT < /dev/tty
    NEW_PORT=${NEW_PORT:-$CURRENT_PORT}
    
    read -p ">> 路径 (path) [当前: ${CURRENT_PATH}]: " NEW_PATH < /dev/tty
    NEW_PATH=${NEW_PATH:-$CURRENT_PATH}
    
    read -p ">> Knock 域名 [当前: ${CURRENT_KNOCK}]: " NEW_KNOCK_DOMAIN < /dev/tty
    NEW_KNOCK_DOMAIN=${NEW_KNOCK_DOMAIN:-$CURRENT_KNOCK}

    read -p ">> 证书(crt/cer)文件路径 [当前: ${CURRENT_CERT}]: " NEW_CERT_FILE < /dev/tty
    NEW_CERT_FILE=${NEW_CERT_FILE:-$CURRENT_CERT}

    read -p ">> 密钥(key)文件路径 [当前: ${CURRENT_KEY}]: " NEW_KEY_FILE < /dev/tty
    NEW_KEY_FILE=${NEW_KEY_FILE:-$CURRENT_KEY}
    
    read -p ">> 后置转发代理地址 (留空则删除) [当前: ${CURRENT_FORWARDER}]: " NEW_FORWARDER < /dev/tty
...
function generate_self_signed_cert() {
    ...
    read -p ">> 请输入密钥(key)文件保存路径 [默认: /root/1.key]: " KEY_PATH < /dev/tty
    KEY_PATH=${KEY_PATH:-/root/1.key}

    read -p ">> 请输入证书(crt)文件保存路径 [默认: /root/1.crt]: " CERT_PATH < /dev/tty
